...
const safe = (v: unknown): Record<string, unknown> => (v && typeof v === 'object' ? (v as any) : {});
...
async function listBills(uid: string): Promise<Bill[]> {
  ...
  return snap.docs.map(d => ({ id: d.id, ...safe(d.data()) })) as Bill[];
}
...
async function listBNPL(uid: string): Promise<BNPLPlan[]> {
  ...
  return snap.docs.map(d => ({ id: d.id, ...safe(d.data()) })) as BNPLPlan[];
}
...
async function listPaychecks(uid: string, fromMs: number, toMs: number): Promise<Paycheck[]> {
  ...
  return snap.docs.map(d => ({ id: d.id, ...safe(d.data()) })) as Paycheck[];
}
