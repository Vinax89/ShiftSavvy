...
async function getPrimaryAccount(uid) {
...
function nextBillOccurrence(bill, now, until) {
    const out = [];
...
async function upsertAlert(a) {
    ...
    const snap = await ref.get();
    if (snap.exists)
        return; // already open/ack; leave it
    const payload = { ...a, state: 'open', createdAt: Date.now(), hash };
...
async function runForUser(uid) {
    ...
    for (const p of bnpl) {
        if (p.status !== 'active' || !p.nextDueAt)
            continue;
    ...
    const debitsCents = bills.flatMap(b => nextBillOccurrence(b, new Date(todayMs), new Date(endMs)).map(x => b.amountCents))
        .reduce((a, c) => a + (c || 0), 0)
    ...
    if (emailEnabled && email) {
        ...
        if (!openAlerts.empty) {
            await sendEmail(email, `ShiftSavvy Alerts`, openAlerts.docs.map(d => d.data()));
        }
    }
}
async function sendEmail(to, subject, alerts) {
...
export const alertsDaily = onSchedule({
    ...
    for (const uid of uids) {
        try {
            ...
            console.error("alertsDaily error for", uid, e);
        }
    }
});
